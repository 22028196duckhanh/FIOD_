...
from yolov9_main.models.common import *
from yolov9_main.models.experimental import *
from yolov9_main.utils.general import LOGGER, check_version, check_yaml, make_divisible, print_args
from yolov9_main.utils.plots import feature_visualization
from yolov9_main.utils.torch_utils import (fuse_conv_and_bn, initialize_weights, model_info, profile, scale_img, select_device,
                              time_sync)
from yolov9_main.utils.tal.anchor_generator import make_anchors, dist2bbox
...
class DualDDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            d2.append(torch.cat((self.cv4[i](x[self.nl+i]), self.cv5[i](x[self.nl+i])), 1))
        if self.training:
            print("d1 len ", len(d1)) #3 detect layer
            print("d2 len ", len(d2)) #3
            return [d1, d2]
        elif self.dynamic or self.shape != shape:
            self.anchors, self.strides = (d1.transpose(0, 1) for d1 in make_anchors(d1, self.stride, 0.5))
            self.shape = shape
